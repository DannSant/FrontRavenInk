...
export class InventoryService {
  ...
  getInventoryItemsAll(){
    ...
    saveItem(item:InventoryItem){
      ...
      if(item.id){
        url = url + "/" + item.id;
        return this.http.put(url,item,{headers}).pipe(catchError(e =>{
          console.log(e);
          //let errorNumber:number = e.error.error.errno;
          this._alert.showAlert("Error","Ha ocurrido un error al guardar el producto, intentar de nuevo mas tarde ","error");
          return of(e)
        }));
      }else {

        return this.http.post(url,item,{headers}).pipe(catchError(e =>{
          console.log(e);
          //let errorNumber:number = e.error.error.errno;
          this._alert.showAlert("Error","Ha ocurrido un error al guardar el producto, intentar de nuevo mas tarde ","error");
          return of(e)
    ...
    deleteItem(id:string){
      ...
      let url = SERVICE_URL + "/inventory/" + id;
      return this.http.delete(url,{headers}).pipe(catchError(e =>{
        console.log(e);
        //let errorNumber:number = e.error.error.errno;
        this._alert.showAlert("Error","Ha ocurrido un error al guardar el producto, intentar de nuevo mas tarde ","error");
        return of(e)
